...
if __name__ == "__main__":
    ...
    # if args.softmax:
    #     save_dir = "ckpt/phase_separation/{}_lr{}_bs{}_epoch{}_softmax".format(
    #         args.backbone, args.lr, args.batch_size, args.max_epochs
    #     )
    # else:
    #     save_dir = "ckpt/phase_separation/{}_lr{}_{}_bs{}_epoch{}_arcface_m{}_s{}".format(
    #         args.backbone, args.lr_base, args.lr, args.batch_size, args.max_epochs, args.m, args.s
    #     )

    if args.softmax:
        save_dir = "ckpt/wrinkle/{}_lr{}_bs{}_epoch{}_softmax".format(
            args.backbone, args.lr, args.batch_size, args.max_epochs
        )
    else:
        save_dir = "ckpt/wrinkle/{}_lr{}_{}_bs{}_epoch{}_arcface_m{}_s{}".format(
            args.backbone, args.lr_base, args.lr, args.batch_size, args.max_epochs, args.m, args.s
        )
    ...
    if not args.softmax:
        arcface_cls_ids = [id(item) for item in model.arcface_cls.parameters()]
        other_params = [param for param in model.parameters() if id(param) not in arcface_cls_ids]

        model_params = [
            {'params': model.arcface_cls.parameters(), 'lr': args.lr},
            {'params': other_params, 'lr': args.lr_base}
        ]
    else:
